...
def collect_kalm_embedding_for_nntrainer(params, n_layers, dtype):
    ...
    # KALM uses "0.auto_model." prefix instead of "model."
    prefix = "0.auto_model."

    add("embedding0:weight", params[f"{prefix}embed_tokens.weight"])

    for i in range(n_layers):
        lp = f"{prefix}layers.{i}."  # HF layer prefix
    ...
    # No output_of_causallm — embedding-only model
    return named
...
if __name__ == "__main__":
    ...
    if output_name.endswith('.safetensors'):
        ...
        named = collect_kalm_embedding_for_nntrainer(model.state_dict(), config.num_hidden_layers, data_dtype)
        write_safetensors(output_name, named)
        verify_safetensors(output_name)
    else:
        with open(output_name, "wb") as f:
            save_kalm_embedding_for_nntrainer(model.state_dict(), config.num_hidden_layers, data_dtype, f)
